...
if [ "$#" -le 1 ];then
	echo "Usage: $0 <android-8.1> liquid '# of jobs' 'version' "
...
if [ "$rom" == "liquid" ];then
	repo init -u https://github.com/LiquidRemix/manifest.git -b oc-mr2-gsi
fi
...
elif [[ $vers == "arm-aonly" ]];then
buildVariant treble_arm_avN-userdebug arm-aonly
elif [[ $vers == "arm-ab" ]];then
buildVariant treble_arm_bvN-userdebug arm-ab
fi
